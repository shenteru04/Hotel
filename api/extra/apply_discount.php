...
// Authenticate user (only admin/receptionist can apply discounts)
...
if (!is_numeric($amount) || $amount <= 0) {
    http_response_code(400);
    echo json_encode(apiResponse('error', 'Invalid discount amount'));
    exit;
}
...
try {
    ...
    echo json_encode(apiResponse('success', 'Discount applied successfully', [
        'billing' => $billing,
        'discount' => [
            'description' => $description,
            'amount' => $amount
