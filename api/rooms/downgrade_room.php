...
// Authenticate user (only admin/receptionist can downgrade rooms)
...
try {
    ...
    if ($newRoomLevel >= $currentRoomLevel) {
        $conn->rollBack();
        http_response_code(400);
        echo json_encode(apiResponse('error', 'New room must be lower category than current room'));
        exit;
    }
    ...
    $priceDifference = ($newRoom['price'] - $reservation['current_price']) * $nights; // Will be negative
    ...
    echo json_encode(apiResponse('success', 'Room downgraded successfully', $response));
